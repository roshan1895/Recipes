...
//                    Log.e("appbarexpanded",appBarExpanded+"");
    ...
    {   if(errorLay.getVisibility()==View.VISIBLE)
        ...
        Call<MealDetailsResponse> call= ApInterface.getFoodClient().create(UrlRequest.class).mealDetails(mealId);
    ...
    {
        Meal meal=mealDetailsResponse.getMeals().get(0);
        if(checkValue(meal.getStrYoutube()))
        {
        ...
        Glide.with(getApplicationContext()).load(meal.getStrMealThumb()).into(htab_header);

        mealTitle.setText(meal.getStrMeal());
      mealCategory.setText(meal.getStrCategory());
      meal_type.setVisibility(View.GONE);
      setInstructions(mealDetailsResponse.getMeals().get(0).getStrInstructions());
      parseIngredients(meal);

    }
    void parseIngredients(Meal meal)
    {
        ...
        if(checkValue(meal.getStrIngredient16()))
        {
            addIngredient(meal.getStrIngredient16(),meal.getStrMeasure16());
        }
        if(checkValue(meal.getStrIngredient17()))
        {
            addIngredient(meal.getStrIngredient17(),meal.getStrMeasure17());
        }
        if(checkValue(meal.getStrIngredient18()))
        {
            addIngredient(meal.getStrIngredient18(),meal.getStrMeasure18());
        }
        if(checkValue(meal.getStrIngredient19()))
        {
            addIngredient(meal.getStrIngredient19(),meal.getStrMeasure19());
        }
        if(checkValue(meal.getStrIngredient20()))
        {
            addIngredient(meal.getStrIngredient20(),meal.getStrMeasure20());
        }
    ...
    {   ingredientsAdapter.add(new Ingredient(name, Constants.getFoodIngredientsSmallImage(name),qty));
    }
